...
const COLOR_PALETTE = [
	"#FF0000", "#FFD700", "#00FF00", "#FFFFFF",
	"#0000FF", "#FF6B00", "#9B59B6", "#E91E63",
	"#00BCD4", "#FF5722", "#8BC34A", "#FFC107",
		...
		<div className="flex-[2] min-w-0 bg-editor-panel border border-foreground/10 rounded-2xl flex flex-col shadow-xl h-full overflow-hidden">
			<div className="flex-1 min-h-0 p-4 overflow-y-auto custom-scrollbar">
				<div className="mb-6">
					...
					{/* Type Selector — 4 types (input removed) */}
					<Tabs
					...
					>
						<TabsList className="mb-4 bg-foreground/5 border border-foreground/5 p-1 w-full grid grid-cols-4 h-auto rounded-xl">
							<TabsTrigger
								value="text"
								className="data-[state=active]:bg-[#2563EB] data-[state=active]:text-white text-muted-foreground py-2 rounded-lg transition-all gap-1.5"
							>
						...
						<TabsContent value="text" className="mt-0 space-y-4">
							...
							<div className="space-y-4">
								...
								<div className="flex items-center justify-between gap-2">
									<ToggleGroup type="multiple" className="justify-start bg-foreground/5 p-1 rounded-lg border border-foreground/5">
										<ToggleGroupItem
											value="bold"
									...
									<ToggleGroup type="single" value={annotation.style.textAlign} className="justify-start bg-foreground/5 p-1 rounded-lg border border-foreground/5">
										<ToggleGroupItem value="left" aria-label={t("annotations.alignLeft")} onClick={() => onStyleChange({ textAlign: "left" })} className="h-8 w-8 data-[state=on]:bg-[#2563EB] data-[state=on]:text-white text-muted-foreground hover:bg-foreground/5 hover:text-foreground">
											<AlignLeft className="h-4 w-4" />
										</ToggleGroupItem>
										<ToggleGroupItem value="center" aria-label={t("annotations.alignCenter")} onClick={() => onStyleChange({ textAlign: "center" })} className="h-8 w-8 data-[state=on]:bg-[#2563EB] data-[state=on]:text-white text-muted-foreground hover:bg-foreground/5 hover:text-foreground">
											<AlignCenter className="h-4 w-4" />
										</ToggleGroupItem>
										<ToggleGroupItem value="right" aria-label={t("annotations.alignRight")} onClick={() => onStyleChange({ textAlign: "right" })} className="h-8 w-8 data-[state=on]:bg-[#2563EB] data-[state=on]:text-white text-muted-foreground hover:bg-foreground/5 hover:text-foreground">
								...
								<div className="grid grid-cols-2 gap-4">
									<div>
										<label className="text-xs font-medium text-foreground mb-2 block">{t("annotations.textColor")}</label>
										<Popover>
											<PopoverTrigger asChild>
												<Button variant="outline" className="w-full h-9 justify-start gap-2 bg-foreground/5 border-foreground/10 hover:bg-foreground/10 px-2">
													<div className="w-4 h-4 rounded-full border border-foreground/20" style={{ backgroundColor: annotation.style.color }} />
													<span className="text-xs text-muted-foreground truncate flex-1 text-left">{annotation.style.color}</span>
									...
									<div>
										<label className="text-xs font-medium text-foreground mb-2 block">{t("annotations.background")}</label>
										<Popover>
											<PopoverTrigger asChild>
												<Button variant="outline" className="w-full h-9 justify-start gap-2 bg-foreground/5 border-foreground/10 hover:bg-foreground/10 px-2">
											...
											<PopoverContent className="w-[260px] p-3 bg-editor-surface-alt border border-foreground/10 rounded-xl shadow-xl">
												...
												/>
												<Button variant="ghost" size="sm" className="w-full mt-2 text-xs h-7 hover:bg-foreground/5 text-muted-foreground" onClick={() => onStyleChange({ backgroundColor: "transparent" })}>
						...
						<TabsContent value="image" className="mt-0 space-y-4">
							...
							<Button onClick={() => fileInputRef.current?.click()} variant="outline" className="w-full gap-2 bg-foreground/5 text-foreground border-foreground/10 hover:bg-[#2563EB] hover:text-white hover:border-[#2563EB] transition-all py-8">
							...
							<p className="text-xs text-muted-foreground/70 text-center leading-relaxed">{t("annotations.supportedFormats")}</p>
						</TabsContent>

						{/* ── Figure / Arrow ───────────────────────────────── */}
						<TabsContent value="figure" className="mt-0 space-y-4">
							<div>
								<label className="text-xs font-medium text-foreground mb-3 block">{t("annotations.arrowDirection")}</label>
								<div className="grid grid-cols-4 gap-2">
									{(["up", "down", "left", "right", "up-right", "up-left", "down-right", "down-left"] as ArrowDirection[]).map((direction) => {
										const ArrowComponent = getArrowComponent(direction);
										return (
											<button
												key={direction}
												onClick={() => onFigureDataChange?.({ ...annotation.figureData!, arrowDirection: direction })}
												aria-label={t("annotations.arrowDirectionOption", "Arrow direction: {{direction}}", { direction: direction.replace(/-/g, " ") })}
												className={cn(
											...
											>
												<ArrowComponent color={annotation.figureData?.arrowDirection === direction ? "#ffffff" : "#94a3b8"} strokeWidth={3} />
											</button>
										);
									})}
								</div>
							...
							<div>
								...
								<Slider
									...
									onValueChange={([value]) => onFigureDataChange?.({ ...annotation.figureData!, strokeWidth: value })}
									min={1} max={6} step={1} className="w-full"
								/>
							</div>
							<div>
								<label className="text-xs font-medium text-foreground mb-2 block">{t("annotations.arrowColor")}</label>
								<Popover>
									<PopoverTrigger asChild>
										<Button variant="outline" className="w-full h-10 justify-start gap-2 bg-foreground/5 border-foreground/10 hover:bg-foreground/10">
											<div className="w-5 h-5 rounded-full border border-foreground/20" style={{ backgroundColor: annotation.figureData?.color || "#2563EB" }} />
											<span className="text-xs text-muted-foreground truncate flex-1 text-left">{annotation.figureData?.color || "#2563EB"}</span>
											<ChevronDown className="h-3 w-3 opacity-50" />
										</Button>
									</PopoverTrigger>
									<PopoverContent className="w-[260px] p-3 bg-editor-surface-alt border border-foreground/10 rounded-xl shadow-xl">
										<Block
											...
											colors={COLOR_PALETTE}
											onChange={(color) => onFigureDataChange?.({ ...annotation.figureData!, color: color.hex })}
											style={{ borderRadius: "8px" }}
										/>
									</PopoverContent>
								</Popover>
						...
						<TabsContent value="blur" className="mt-0 space-y-4">
							<div className="p-4 bg-foreground/5 rounded-xl border border-foreground/10 flex flex-col items-center">
								<div className="w-full space-y-3">
									...
									<Slider value={[annotation.blurIntensity ?? 20]} onValueChange={([value]) => onBlurIntensityChange?.(value)} min={1} max={100} step={1} className="w-full" />
								</div>
								<div className="w-full space-y-3 mt-4">
									<div className="flex items-center justify-between">
										<span className="text-xs font-medium text-foreground">{t("annotations.solidColor", "Solid Color (Censorship)")}</span>
									</div>
									<div className="flex flex-wrap gap-2">
										<button
											onClick={() => onBlurColorChange?.("")}
											className={cn("w-8 h-8 rounded-full border-2 flex items-center justify-center transition-all", !annotation.blurColor || annotation.blurColor === "transparent" ? "border-[#2563EB] scale-110" : "border-transparent hover:border-foreground/20")}
										...
										<button onClick={() => onBlurColorChange?.("#000000")} className={cn("w-8 h-8 rounded-full border-2 transition-all bg-black", annotation.blurColor === "#000000" ? "border-[#2563EB] scale-110" : "border-transparent hover:border-foreground/20")} title="Black" />
										<button onClick={() => onBlurColorChange?.("#FFFFFF")} className={cn("w-8 h-8 rounded-full border-2 transition-all bg-white", annotation.blurColor === "#FFFFFF" ? "border-[#2563EB] scale-110" : "border-transparent hover:border-foreground/20")} title="White" />
										<Popover>
											<PopoverTrigger asChild>
												<button
													className={cn("w-8 h-8 rounded-full border-2 transition-all flex items-center justify-center overflow-hidden relative", annotation.blurColor && !["#000000", "#FFFFFF", "transparent", ""].includes(annotation.blurColor) ? "border-[#2563EB] scale-110" : "border-transparent hover:border-foreground/20")}
											...
											<PopoverContent className="w-[260px] p-3 bg-editor-surface-alt border border-foreground/10 rounded-xl shadow-xl">
												<Block color={annotation.blurColor || "#2563EB"} colors={COLOR_PALETTE} onChange={(color) => onBlurColorChange?.(color.hex)} style={{ borderRadius: "8px" }} />
											</PopoverContent>
										</Popover>
									</div>
								</div>
